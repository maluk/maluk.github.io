...
V2 launches only when all 50 state engines (and DC if ready) have verified official sources, each jurisdiction has golden and boundary tests, mandatory statewide employee deductions are accounted for, the complete calculator UI is tested on desktop and mobile (completed September 23, 2026), and the live static pages and legacy URL behavior pass direct HTTP checks. HTTP 301 redirects require a hosting change or redirect-capable proxy.
