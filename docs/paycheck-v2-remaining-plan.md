...
Status on 2026-09-23: the branch exports the homepage, all 50 current-year state pages, and four 2025 historical pages. It has 131 passing calculation tests. The branch is not deployed; CI currently uploads the static build as an artifact. The V2 definition of done is not yet met.
...
**PAY-002 — Audit the implemented rules.** California 2025 and 2026 now use EDD's four pay-period Method B tables. Official Examples B–D and bracket boundaries pass. Recheck the 2026 FL, NY, TX, WA, AK, NV, SD, TN, NH, WY, PA, IL, and IN modules and the 2025 FL, TX, and WA modules against each jurisdiction's primary payroll source. Add a named official-example golden test for every jurisdiction/year where one exists, plus zero-wage, high-wage, wage-cap, and final-paycheck crossing tests for applicable programs. Check deduction wage treatment by state, especially 401(k), HSA, and cafeteria benefits. Any rule that cannot be substantiated returns to `pending_review` and leaves the sitemap until corrected.
...
**PAY-008 — Production deployment.** The user chose the existing GitHub Pages site. `npm run deploy` now tests, builds, verifies, and publishes to its `gh-pages` branch from a clean checkout. The export includes the custom-domain `CNAME`, `.nojekyll` for Next assets, and legacy year URL refresh pages. GitHub Pages cannot provide HTTP 301 redirects, so the original 301 acceptance criterion remains unmet. After release review, deploy and verify the homepage, all representative state pages, 404, canonical, legacy URL behavior, and sitemap. Submit the sitemap after the live checks.
...
V2 launches only when all 50 state engines (and DC if ready) have verified official sources, each jurisdiction has golden and boundary tests, mandatory statewide employee deductions are accounted for, the complete calculator UI is tested on desktop and mobile, and the live static pages and legacy URL behavior pass direct HTTP checks. HTTP 301 redirects require a hosting change or redirect-capable proxy.
