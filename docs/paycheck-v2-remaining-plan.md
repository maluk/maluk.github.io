...
**PAY-008 — Production deployment.** A manual GitHub Actions workflow now tests, builds, and uploads `out/` through Cloudflare Wrangler. Create a Cloudflare Pages Direct Upload project named `thetax-us`, set its production branch to `master`, and configure `CLOUDFLARE_API_TOKEN` and `CLOUDFLARE_ACCOUNT_ID` repository secrets. Cloudflare account access is unavailable in this workspace, so the project, domain attachment, and live deployment remain pending. After deployment, verify HTTP 301 responses for the five existing `/state/2026/` duplicates, check 404 and canonical behavior, then submit the sitemap. Keep production changes behind the normal release review.
